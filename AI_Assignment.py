...
# Additional user inputs for filtering by rating range and top N results
top_n = st.text_input('Enter the number of top results (leave empty for all):')
...
if 'Number of Reviews' not in google_review_data.columns:
    google_review_data['Number of Reviews'] = google_review_data['Review'].apply(lambda x: len(x.split()))  
if 'Number of Reviews' not in tripadvisor_data.columns:
    tripadvisor_data['Number of Reviews'] = tripadvisor_data['Review'].apply(lambda x: len(x.split()))  
...
filtered_data = place_df[(place_df['Combined Rating'] >= min_rating) & (place_df['Combined Rating'] <= max_rating)]

# Sort the data by total reviews and combined rating (popularity-based filtering)
sorted_data = filtered_data.sort_values(by=['Total Reviews', 'Combined Rating'], ascending=[False, False])

# If top_n is specified, select the top N results
if top_n:
    sorted_data = sorted_data.head(int(top_n))

# Display the dataframe in Streamlit
popular_restaurants = sorted_data[['Restaurant', 'Location', 'Total Reviews', 'Combined Rating']]
